...
f = uproot.open(
    "/sps/km3net/users/alflazo/dstProd/v7.1_v7.2_jsh/mcv7.2.gsg_muon-CC_1-100GeV.km3sim.jorcarec.jsh.aanet.dst_merged.root"
)  # nu mu CC
...
h1 = bh.Histogram(
    bh.axis.Regular(30, 1, 100, transform=bh.axis.transform.log),
)
h2 = bh.Histogram(
    bh.axis.Regular(30, 1, 100, transform=bh.axis.transform.log),
)
...
fig, ax = plt.subplots()
h = compute_chi2_map(E, dir_z, osci_w, E, dir_z, osci_w_bad)
plothist2d(h, ax)
plt.xscale("log")
plt.show()
print("$" * 20)
print("Standard histogramming sensitivity: ", h.sum())
print("$" * 20)

fig, ax = plt.subplots()
h = compute_chi2_map(E, dir_z, osci_w, E, dir_z, osci_w_bad, use_transform=True)
plothist2d(h, ax)
plt.xscale("log")
plt.show()
print("$" * 20)
print("Homogeneous histogramming sensitivity: ", h.sum())
print("$" * 20)